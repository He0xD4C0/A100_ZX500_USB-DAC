...
#include <malloc.h>
#include <dmp_logo.h>
#endif /* CONFIG_VIDEO_BMP_DMP_LOGO */
...
{
	return video_display_bitmap_to_fb(bmp_image, x, y, (ulong)video_fb_address);
}
int video_display_bitmap_to_fb(ulong bmp_image, int x, int y, ulong fb_addr)
#else
int video_display_bitmap(ulong bmp_image, int x, int y)
#endif
{
...
#ifdef CONFIG_VIDEO_BMP_DMP_LOGO
	if (fb_addr == 0) {
		pr_err("[boot logo] Cannot display at fb_addr = 0\n");
		return 1;
	}

	printf("[boot logo] Display fb 0x%lx\n", fb_addr);
#endif
...
#ifdef CONFIG_VIDEO_BMP_DMP_LOGO
static int load_logo_bmp(const char* file_name, const char *env_name)
{
	struct blk_desc *dev_desc = NULL;
	disk_partition_t info;
	int ret;
	loff_t file_len, read_len;
	char *buf;

	dev_desc = blk_get_dev("mmc", mmc_get_env_dev());
	if (!dev_desc) {
		pr_err("[boot logo] blk_get_dev failed.\n");
		return -1;
	}

	ret = part_get_info_by_name(dev_desc, "boot_logo", &info);
	if (ret < 0) {
		pr_err("[boot logo] part_get_info_by_name failed.\n");
		return -1;
	}
	env_set_hex("boot_logo_part", (ulong)ret);
	ext4fs_set_blk_dev(dev_desc, &info);

	ret = ext4fs_mount(info.size);
	if (!ret) {
		pr_err("[boot logo] ext4fs_mount failed.\n");
		return -1;
	}

	ret = ext4fs_open(file_name, &file_len);
	if (ret < 0) {
		pr_err("[boot logo] file %s not found.\n", file_name);
		return -1;
	}

	buf = (char *)malloc(file_len);
	if(!buf) {
		pr_err("[boot logo] malloc failed.\n");
		ext4fs_close();
		return -1;
	}

	env_set_addr(env_name, (void *)buf);

	ret = ext4fs_read(buf, 0, file_len, &read_len);
	ext4fs_close();
	if (ret == -1) {
		pr_err("[boot logo] ext4fs_read failed.\n");
		return -1;
	}

	printf("[boot logo] Display %s \n", file_name);

	return 0;
}

static int load_logo_bmp_offset(const char* part_name, const ulong offset,
	const ulong count, const char *env_name)
{
	struct blk_desc *dev_desc = NULL;
	int ret = 0;
	...
	if (ret < 0) {
		pr_err("[boot logo] part_get_info_by_name %s failed.\n", part_name);
		return -1;
	}

	buf = (void *)malloc(CONFIG_SYS_VIDEO_LOGO_MAX_SIZE);
	if(!buf) {
		pr_err("[boot logo] malloc failed.\n");
		return -1;
	}

	if (count * 512 > CONFIG_SYS_VIDEO_LOGO_MAX_SIZE) {
		pr_err("[boot logo] bmp count too large. Supported to %d bytes\n",
			CONFIG_SYS_VIDEO_LOGO_MAX_SIZE);
		return -1;
	}

	if (count + offset > part_info.size) {
		printf("[boot logo] Read out of range\n");
		return -1;
	}

	if (blk_dread(dev_desc, part_info.start + offset, count, buf) != count) {
		printf("[boot logo] Error reading blocks\n");
		return -1;
	}

	env_set_addr(env_name, (void *)buf);

	printf("[boot logo] Display logo from %s offset %ld \n", part_name, offset);

	return 0;
}

static int check_logo_pos(const int x, const int y)
{
	if (((x != BMP_ALIGN_CENTER) && (x > VIDEO_VISIBLE_COLS)) || x < 0 ) {
		printf("[boot logo] invalid pos x %d\n", x);
		return -1;
	}
	if (((y != BMP_ALIGN_CENTER) && (y > VIDEO_VISIBLE_COLS)) || y < 0 ) {
		printf("[boot logo] invalid pos y %d\n", y);
		return -1;
	}
	return 0;
}
...
int display_dmp_logo_file(const char* file_name)
{
	...
	ret = load_logo_bmp(file_name, "dmp_logo_file");
	if (ret)
		return ret;

	ret = check_logo_pos(video_logo_xpos, video_logo_ypos);
	if (ret)
		return ret;
	...
	ret = video_display_bitmap_to_fb(env_get_hex("dmp_logo_file", 0),
		video_logo_xpos, video_logo_ypos, (ulong)video_fb_address);

	return ret;
}

int display_dmp_logo_offset(const char* part_name, const ulong offset,
	const ulong count)
{
	int ret;

	ret = load_logo_bmp_offset(part_name, offset, count,
		"dmp_logo_offset");
	if (ret)
		return ret;
	...
	ret = video_display_bitmap_to_fb(env_get_hex("dmp_logo_offset", 0),
		video_logo_xpos, video_logo_ypos, (ulong)video_fb_address);
...
static void *video_logo(void)
{
...
#ifdef CONFIG_VIDEO_BMP_DMP_LOGO
	uint64_t fb_addr_kernel_logo = CONFIG_FB_KERNEL_LOGO_ADDR_2G;

	if (gd->ram_size > (uint64_t)PHYS_SDRAM_SIZE) {
		fb_addr_kernel_logo = CONFIG_FB_KERNEL_LOGO_ADDR_4G;
	}
#endif
	...
	if (load_logo_bmp(env_get("boot_logo"), "splashimage"))
		goto plot_logo_h;
	if (video_display_bitmap_to_fb(env_get_hex("splashimage", 0),
				video_logo_xpos,
				video_logo_ypos, (ulong)video_fb_address))
		goto plot_logo_h;
	if (fb_addr_kernel_logo != 0) {
		memset((void *)fb_addr_kernel_logo, 0, pGD->memSize);
		if (load_logo_bmp(env_get("kernel_logo"), "kernelimage"))
			goto displayed_boot_logo;
		if (video_display_bitmap_to_fb(env_get_hex("kernelimage", 0),
					video_logo_xpos,
					video_logo_ypos, fb_addr_kernel_logo))
			goto displayed_boot_logo;
	}
displayed_boot_logo:
	video_logo_height = 0;
