			     ...
			     ulong *os_data, ulong *os_len)
{
	...
	if (bootmode) {
		snprintf(newbootargs, sizeof(newbootargs), " androidboot.mode=%s", bootmode);

		if (strlen(newbootargs) + 1 >
				(sizeof(commandline) - strlen(commandline)))
			printf("The '%s' is too long to be appended to bootargs\n", newbootargs);
		else
			strncat(commandline, newbootargs, sizeof(commandline) - strlen(commandline));

#ifdef TARGET_BUILD_VARIANT_USERDEBUG
		if ((strncmp(bootmode, "diag", sizeof("diag")) == 0) ||
		    (strncmp(bootmode, "init", sizeof("init")) == 0)) {
			snprintf(newbootargs, sizeof(newbootargs), " androidboot.selinux=permissive");
	...
	char kernel_logo_args[128] = "vt.global_cursor_default=0";
	if (strlen(kernel_logo_args) + 2 >
			(sizeof(commandline) - strlen(commandline))) {
		printf("The 'kernel_logo_args' is too long to be appended to bootargs\n");
	} else {
		strncat(commandline, " ", sizeof(commandline) - strlen(commandline));
		strncat(commandline, kernel_logo_args, sizeof(commandline) - strlen(commandline));
	}
#endif

#ifdef CONFIG_ICX_DEAD_BATTERY
	unsigned long voltage;
	voltage = env_get_ulong("battery_voltage", 10, 0);
	if (voltage) {
		snprintf(newbootargs, sizeof(newbootargs), " bq25898_icx_charger.dead_battery=%lu", voltage);
