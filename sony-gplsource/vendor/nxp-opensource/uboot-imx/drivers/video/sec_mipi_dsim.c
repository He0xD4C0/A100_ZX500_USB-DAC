		       ...
		       u8 data_type, const u8 *buf, int len)
{
	...
	sec_mipi_dsim_config_cmd_lpm(dsim, true);
...
static void sec_mipi_dsim_set_main_mode(struct sec_mipi_dsim *dsim)
{
	...
	const uint32_t bit_clk =  792000000;
	const uint32_t pixel_clk = 66000000;
	...
	if (dsim->dsi_panel_drv && vmode->xres == 720 && vmode->yres == 1280) {
		hfp_wc = vmode->right_margin * (bit_clk / 8) / pixel_clk / 2 - 2;
		hbp_wc = vmode->left_margin * (bit_clk / 8) / pixel_clk - 8;
	}
	...
	if (dsim->dsi_panel_drv && vmode->xres == 720 && vmode->yres == 1280)
		hsa_wc = vmode->hsync_len * (bit_clk / 8) / pixel_clk - 2;
...
static void sec_mipi_dsim_config_dphy(struct sec_mipi_dsim *dsim)
{
	...
	u32 phytiming = 0, phytiming1 = 0, phytiming2 = 0, timeout = 0;
	u32 hactive, vactive;
	struct fb_videomode *vmode = &dsim->vmode;
	 ...
	 */
	if (dsim->lanes == 2 &&
	    vmode->xres == 1280 && vmode->yres == 720 &&
	    vmode->refresh == 60) {
		key.bit_clk >>= 1;
	}
	...
	printf("phytiming 0x%x, phytiming1 0x%x, phytiming2 0x%x\n",
	       phytiming, phytiming1, phytiming2);
...
static int sec_mipi_dsim_bridge_enable(struct mipi_dsi_bridge_driver *bridge_driver)
{
	int ret;
	struct sec_mipi_dsim *dsim_host = (struct sec_mipi_dsim *)bridge_driver->driver_private;
	struct fb_videomode *vmode = &dsim_host->vmode;
	...
	if (vmode->xres == 720 && vmode->yres == 1280) {
	...
	struct fb_videomode *fbmode)
{
	...
	if (dsim_host->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE) {
		 ...
		 */
		if(fbmode->xres == 720 && fbmode->yres == 1280)
			dsim_host->pms = 0x6599;//for 720p 66Mhz
		else if (fbmode->xres == 480 && fbmode->yres == 800)
			dsim_host->pms = 0x61a8;//for WVGA 40Mhz
		else
			dsim_host->pms = 0x4210;//default for 1080p
	}
