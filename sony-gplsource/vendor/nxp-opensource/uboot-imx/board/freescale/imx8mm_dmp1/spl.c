...
void spl_dram_init(void)
{
	...
	/* ddr train */
	dram_timing.status = 0;
	ddr_init(&dram_timing);
	if (dram_timing.status < 0) {
		printf("[DRAM] try 1CS\n");
		ddr_init(&dram_timing_1cs);
	}
#else
	ddr_init(&dram_timing);
#endif
...
#if (defined(CONFIG_ICX_SDCARD_POWER_DOWN_RESET))
			gpio_direction_output(USDHC2_PWR_GPIO, 0);
			udelay(500);
#endif /* (defined(CONFIG_ICX_SDCARD_POWER_DOWN_RESET)) */
...
int power_init_board(void)
{
	...
	/* BD71837 bug workaround for rev.A0 */
  {
	...
	if (pmic_reg_read(p, BD71837_REV, &regval) != 0)
	...
	do {
		if (regval == 0xa0) {
			...
			(void) pmic_reg_read(p, BD71837_REV, &regval);
			if (regval != 0xa0) {
	...
	/* check LDO4/LDO6 and recovery */
	ret = pmic_reg_read(p, BD71837_LDO4_VOLT, &val);
	if (ret != 0)
		printf("--> Fail to read LDO4 register(0x1B)\n");

	printf("LDO4 0x%x\n", val);
	if (val & 0x80 /* LDO4_SEL */) {
		printf("LDO4 need turn on.\n");
		ret = pmic_reg_write(p, BD71837_LDO4_VOLT, 0x40);
		if (ret != 0)
			printf("--> Fail to write LDO4 register(0x1B)\n");
		udelay(10000); /* wait a stable of switching regulator */
	}

	ret = pmic_reg_read(p, BD71837_LDO6_VOLT, &val);
	if (ret != 0)
		printf("--> Fail to read LDO6 register(0x1D)\n");

	printf("LDO6 0x%x\n", val);
	if (val & 0x80 /* LDO6_SEL */) {
		printf("LDO6 need turn on.\n");
		ret = pmic_reg_write(p, BD71837_LDO6_VOLT, 0x43);
		if (ret != 0)
			printf("--> Fail to write LDO6 register(0x1D)\n");
	}

	/* increase VDD_SOC to typical value 0.82v before first DRAM access */
	pmic_reg_write(p, BD71837_BUCK1_VOLT_RUN, 0x0c);
	pmic_reg_write(p, BD71837_BUCK1_VOLT_IDLE, 0x0c);
	pmic_reg_write(p, BD71837_BUCK1_VOLT_SUSP, 0x0c);
...
#else
	/* increase NVCC_DRAM_1V2 to 1.12v for DDR4 */
	pmic_reg_write(p, BD71837_BUCK8_VOLT, 0x20);
#endif

	/* decrease VDD_3.3 to 3.2v */
	pmic_reg_write(p, BD71837_BUCK6_VOLT, 0x2);
