...
/*
 * Copyright (c) 2017,2018 Sony Video & Sound Products Inc.
 * 
 * Ignore filesystem security restriction for Android
 * Added op_open_with_fd()
 * Added op_device_with_desc()
 */
...
static libusb_device* op_device_with_desc(libusb_context *ctx,
		const char *dev_node, const unsigned char* desc, int desc_length) {
	uint8_t busnum, devaddr;
	unsigned int session_id;
	if (linux_get_device_address(ctx, 0, &busnum, &devaddr, dev_node, NULL)
			!= LIBUSB_SUCCESS) {
	...
	dev->device_address = devaddr;
	struct linux_device_priv* priv = _device_priv(dev);
	int descriptors_size = 1024; /* Begin with a 1024 byte alloc */
	while (descriptors_size < desc_length) {
		descriptors_size *= 2;
	}
	priv->descriptors = usbi_reallocf(priv->descriptors, descriptors_size);
	memcpy(priv->descriptors, desc, desc_length);
	priv->descriptors_len = desc_length;
...
const struct usbi_os_backend linux_usbfs_backend = {
	...
	.open = op_open,
	.open_with_fd = op_open_with_fd,
	.device_with_desc = op_device_with_desc,
	.close = op_close,
