...
.PHONY: clean
clean:
	@rm -f $(MKIMG) u-boot-atf.bin u-boot-atf-tee.bin u-boot-spl-ddr.bin u-boot.itb u-boot-4g.itb u-boot-ivt.itb u-boot.its u-boot-ddr3l.itb u-boot-ddr3l.its u-boot-spl-ddr3l.bin u-boot-ddr4.itb u-boot-ddr4.its u-boot-spl-ddr4.bin u-boot-ddr4-evk.itb u-boot-ddr4-evk.its $(OUTIMG)
...
u-boot-4g.itb: $(dtbs)
	./$(PAD_IMAGE) bl31.bin
	TEE_LOAD_ADDR=$(TEE_4G_LOAD_ADDR) ATF_LOAD_ADDR=$(ATF_LOAD_ADDR) ./mkimage_fit_atf.sh $(dtbs) > u-boot-4g.its
	./mkimage_uboot -E -p 0x3000 -f u-boot-4g.its u-boot-4g.itb
	rm -f u-boot-4g.its
...
flash_evk_no_hdmi_4g: $(MKIMG) u-boot-spl-ddr.bin u-boot-4g.itb
	./mkimage_imx8 -fit -loader u-boot-spl-ddr.bin 0x7E1000 -second_loader u-boot-4g.itb 0x40200000 0x60000 -out $(OUTIMG)

flash_evk_no_hdmi_4g_dual_bootloader: $(MKIMG) u-boot-spl-ddr.bin u-boot-4g.itb
	./mkimage_imx8 -fit -loader u-boot-spl-ddr.bin 0x7E1000 -out $(OUTIMG)
	./mkimage_imx8 -single_fit_loader u-boot-4g.itb 0x40200000 -out u-boot-ivt.itb
	TEE_LOAD_ADDR=$(TEE_4G_LOAD_ADDR) ATF_LOAD_ADDR=$(ATF_LOAD_ADDR) ./print_fit_hab.sh 0x0 $(dtbs)

flash_evk_no_hdmi_dual_bootloader: $(MKIMG) u-boot-spl-ddr.bin u-boot.itb
	./mkimage_imx8 -fit -loader u-boot-spl-ddr.bin 0x7E1000 -out $(OUTIMG)
	./mkimage_imx8 -single_fit_loader u-boot.itb 0x40200000 -out u-boot-ivt.itb
	TEE_LOAD_ADDR=$(TEE_LOAD_ADDR) ATF_LOAD_ADDR=$(ATF_LOAD_ADDR) ./print_fit_hab.sh 0x0 $(dtbs)
...
print_single_fit_hab: u-boot-nodtb.bin bl31.bin $(dtbs)
	./$(PAD_IMAGE) bl31.bin
	TEE_LOAD_ADDR=$(TEE_LOAD_ADDR) ATF_LOAD_ADDR=$(ATF_LOAD_ADDR) ./print_fit_hab.sh 0x0 $(dtbs)

print_single_fit_hab_4g: u-boot-nodtb.bin bl31.bin $(dtbs)
	./$(PAD_IMAGE) bl31.bin
	TEE_LOAD_ADDR=$(TEE_4G_LOAD_ADDR) ATF_LOAD_ADDR=$(ATF_LOAD_ADDR) ./print_fit_hab.sh 0x0 $(dtbs)
