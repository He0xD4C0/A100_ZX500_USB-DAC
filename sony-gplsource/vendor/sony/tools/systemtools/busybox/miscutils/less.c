...
#endif
	ssize_t _eof_error; /* eof if 0, error if < 0 */
	ssize_t readpos;
	ssize_t readeof; /* must be signed */
	const char **buffer;
	const char **flines;
	const char *_empty_line_marker;
	unsigned _num_files;
	unsigned _current_file;
	char *filename;
	char **files;
...
#define less_gets_pos       (G._less_gets_pos    )
...
#define eof_error           (G._eof_error        )
#define readpos             (G.readpos           )
#define readeof             (G.readeof           )
#define buffer              (G.buffer            )
#define flines              (G.flines            )
#define empty_line_marker   (G._empty_line_marker )
#define num_files           (G._num_files         )
#define current_file        (G._current_file      )
...
#define wanted_match        (G._wanted_match     )
#define pattern             (G.pattern           )
#define pattern_valid       (G.pattern_valid     )
#endif
#define terminated          (G._terminated       )
#define term_orig           (G.term_orig         )
#define term_less           (G.term_less         )
#define kbd_input           (G.kbd_input         )
#define INIT_G() do { \
	/*! @note: Using alias pointer globs to avoid LLVM/Clang \
	 *         native code generator issue. \
	 *         LLVM/Clang strictly treats "const" means \
	 *         "NEVER change" at runtime, not meaning that \
	 *         it emits warning or error when assigning a value \
	 *         to variable at compiling time (as GCC). \
	 *         Using barrier() in SET_PTR_TO_GLBALS() macro \
	 *         doesn't fix LLVM/Clang code generator issue. \
	 */ \
	struct globals *globs; \
	globs = (struct globals*)(xzalloc(sizeof(G))); \
	/* @note: Skip checking NULL. */ \
	SET_PTR_TO_GLOBALS(globs); \
	globs->_less_gets_pos = -1; \
	globs->_empty_line_marker = "~"; \
	globs->_num_files = 1; \
	globs->_current_file = 1; \
	globs->_eof_error = 1; \
	globs->_terminated = 1; \
	IF_FEATURE_LESS_REGEXP(globs->_wanted_match = -1;) \
} while (0)
