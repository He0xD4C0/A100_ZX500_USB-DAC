...
int wait_ddrphy_training_complete_with_status(void)
{
	unsigned int mail;
	int rv = -1;
	while (1) {
...
void ddrphy_init_set_dfi_clk(unsigned int drate)
{
	...
	case 3000:
		...
		break;
	case 2800:
		dram_pll_init(DRAM_PLL_OUT_700M);
		dram_disable_bypass();
		break;
	case 2600:
		dram_pll_init(DRAM_PLL_OUT_650M);
		dram_disable_bypass();
		break;
	case 2400:
		...
		break;
	case 2200:
		dram_pll_init(DRAM_PLL_OUT_550M);
		dram_disable_bypass();
		break;
	case 2000:
		dram_pll_init(DRAM_PLL_OUT_500M);
		dram_disable_bypass();
		break;
	case 1800:
		dram_pll_init(DRAM_PLL_OUT_450M);
		dram_disable_bypass();
		break;
	case 1600:
		...
		break;
	case 1400:
		dram_pll_init(DRAM_PLL_OUT_350M);
		dram_disable_bypass();
		break;
	case 1200:
		dram_pll_init(DRAM_PLL_OUT_300M);
		dram_disable_bypass();
		break;
	case 1000:
		dram_pll_init(DRAM_PLL_OUT_250M);
		dram_disable_bypass();
		break;
	case 800:
		dram_pll_init(DRAM_PLL_OUT_200M);
		dram_disable_bypass();
		break;
	case 600:
		dram_pll_init(DRAM_PLL_OUT_150M);
		dram_disable_bypass();
		break;
	case 667:
