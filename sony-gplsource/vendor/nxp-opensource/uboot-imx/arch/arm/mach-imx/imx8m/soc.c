...
#include <fsl_caam.h>
#endif
#ifdef CONFIG_ICX_NVP
#include <nvp.h>
#define ICX_SERIALNO_TYPE_CONVENTIONAL  1
#define ICX_SERIALNO_TYPE_GVA           2
#define ICX_SERIALNO_SIZE_PCD           7
#define ICX_SERIALNO_SIZE_SERIAL        7
#define ICX_SERIALNO_SIZE_CONVENTIONAL  14
#define ICX_SERIALNO_SIZE_GVA           20
#define ICX_NVP_NODE_SIZE_SE2           64
#endif
...
#define SNVS_LPCR_TERN_OFF_POW (BIT(5) | BIT(6))
...
int arch_cpu_init(void)
{
	 ...
	 */
#ifdef CONFIG_DISABLE_BTN_LONG_PRESS
	uint32_t val;
#endif
	...
	val = readl(SNVS_HP_BASE_ADDR + SNVS_LPCR_REG);
	val = val | SNVS_LPCR_BTN_PRESS_TIME;
	writel(val, SNVS_HP_BASE_ADDR + SNVS_LPCR_REG);
#else
	trusty_snvs_update_lpcr(SNVS_LPCR_BTN_PRESS_TIME, 1);
#endif
#endif
	return 0;
}
...
#ifdef CONFIG_ICX_NVP
static int icx_check_serialno(char *serialno, int type)
{
    unsigned int i;
    unsigned int size;

    switch (type) {
    case ICX_SERIALNO_TYPE_CONVENTIONAL:
        size = ICX_SERIALNO_SIZE_CONVENTIONAL;
        break;
    case ICX_SERIALNO_TYPE_GVA:
        size = ICX_SERIALNO_SIZE_GVA;
        /* check serial number type 2 "FFFF...FF" */
        if (!strncmp(serialno, "FFFFFFFFFFFFFFFFFFFF", ICX_SERIALNO_SIZE_GVA)) {
            return -1;
        }
        break;
    default:
    ...
    for (i = 0; i < size; i++) {
        if ((serialno[i] < '0') ||
            ((serialno[i] > '9') && (serialno[i] < 'A')) ||
            ((serialno[i] > 'F') && (serialno[i] < 'a')) ||
             (serialno[i] > 'f')) {
            return -1;
        }
    }
...
{
	u64 serialno = 0;
	char icx_nvp_serial_no[ICX_NVP_NODE_SIZE_SE2];

#ifdef CONFIG_USE_GVA
    memset(serialno, 0, ICX_SERIALNO_SIZE_GVA + 1);

    if (!icx_nvp_get_serialno2(icx_nvp_serial_no, ICX_SERIALNO_SIZE_GVA)) {
        if(!icx_check_serialno(icx_nvp_serial_no, ICX_SERIALNO_TYPE_GVA)) {
			serialno = simple_strtol(icx_nvp_serial_no, NULL, 16);
			goto set_number;
		}
    }
#endif

	memset(icx_nvp_serial_no, 0, ICX_SERIALNO_SIZE_CONVENTIONAL + 1);

    if (!icx_nvp_get_pcode(icx_nvp_serial_no, ICX_SERIALNO_SIZE_PCD + 1)) {
		if (!icx_nvp_get_serialno(&icx_nvp_serial_no[ICX_SERIALNO_SIZE_PCD], ICX_SERIALNO_SIZE_SERIAL)) {
			if (!icx_check_serialno(icx_nvp_serial_no,
					ICX_SERIALNO_TYPE_CONVENTIONAL)) {
				serialno = simple_strtol(icx_nvp_serial_no, NULL, 16);
				goto set_number;
			}
		}
	}
	printf("************* NVP SER ERROR ************\n");
	serialno = 0x000BC614E1234567;

set_number:

	serialnr->low = serialno & 0xFFFFFFFF;
	serialnr->high = serialno >> 32;
}
#else /* nodefined CONFIG_ICX_NVP */
void get_board_serial(struct tag_serialnr *serialnr)
{
	struct ocotp_regs *ocotp = (struct ocotp_regs *)OCOTP_BASE_ADDR;
...
#ifdef CONFIG_IMX_TRUSTY_OS
	trusty_snvs_update_lpcr(SNVS_LPCR_TERN_OFF_POW, 1);
#else
	val = readl(SNVS_HP_BASE_ADDR + SNVS_LPCR_REG);
	val = val | SNVS_LPCR_TERN_OFF_POW;
	writel(val, SNVS_HP_BASE_ADDR + SNVS_LPCR_REG);
#endif
