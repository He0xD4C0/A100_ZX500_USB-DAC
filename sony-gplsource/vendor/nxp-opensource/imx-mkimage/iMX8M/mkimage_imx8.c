...
int main(int argc, char **argv)
{
	...
	unsigned int dcd_size = 0, plugin_start_addr = 0, ap_start_addr = 0, sld_start_addr = 0, sld_src_off = 0, sld_csf_off = 0, sld_load_addr = 0, sfl_start_addr = 0, sfl_csf_off = 0, sfl_load_addr = 0;
	char *ofname = NULL, *hdmi_img = NULL, *dcd_img = NULL, *plugin_img = NULL, *ap_img = NULL, *csf_img = NULL, *csf_plugin_img = NULL, *csf_hdmi_img = NULL, *sld_img = NULL, *sfl_img = NULL;
	...
	static struct option long_options[] =
	{
		...
		{"single_fit_loader", required_argument, NULL, 't'},
		{NULL, 0, NULL, 0}
	};
	...
	while(1)
		...
		switch (c)
		{
			...
			case 'u':
				...
				break;
			case 't':
				fprintf(stderr, "SINGLE FIT LOADER IMAGE:\t%s", optarg);
				sfl_img = optarg;
				if (optind < argc && *argv[optind] != '-') {
					sfl_start_addr = (uint32_t) strtoll(argv[optind++], NULL, 0);
					fprintf(stderr, " start addr: 0x%08x", sfl_start_addr);
				} else {
					fprintf(stderr, "\n-single fit image option require TWO arguments: filename, start address in hex\n\n");
					exit(1);
				}
				break;
			case ':':
	...
	if((sfl_img == NULL) && ((ap_img == NULL) || (ofname == NULL)))
	{
		fprintf(stderr, "mandatory args image and output file name missing! abort\n");
		exit(1);
	}

	/* Generate and insert IVT for single fit loader */
	if (sfl_img != NULL)
	{
		/* Open the output file, create it if not exist. */
		...
		copy_file(ofd, sfl_img, 0, 0, 0);
		/* Generate and insert the IVT. */
		sfl_csf_off = generate_ivt_for_fit(ofd, 0, sfl_start_addr, &sfl_load_addr) + 0x20;
		/* Close the output file. */
		close(ofd);

		/* Dump Single Fit Loader Image */
		fprintf(stderr, "\nSingle Fit Loader:\n");
		fprintf(stderr, " sfl_csf_off \t\t0x%x\n", sfl_csf_off);
		fprintf(stderr, " sfl hab block: \t0x%x 0x%x 0x%x\n",
			sfl_load_addr, 0, sfl_csf_off);
