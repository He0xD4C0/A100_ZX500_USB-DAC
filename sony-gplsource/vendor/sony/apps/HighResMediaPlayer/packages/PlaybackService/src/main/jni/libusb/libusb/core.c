...
/*
 * Copyright (c) 2017 Sony Video & Sound Products Inc.
 * 
 * Added libusb_open_with_fd()
 * Added libusb_get_device_with_desc()
 */
...
int API_EXPORTED libusb_open_with_fd(libusb_device *dev,
	...
	r = usbi_backend->open_with_fd(_handle, fd);
	if (r < 0) {
		usbi_dbg("open %d.%d returns %d", dev->bus_number, dev->device_address,
				r);
		libusb_unref_device(dev);
...
DEFAULT_VISIBILITY
libusb_device * LIBUSB_CALL libusb_get_device_with_desc(libusb_context *ctx,
		const char *dev_node, const unsigned char* desc, int desc_length) {
	if(usbi_backend->device_with_desc == NULL) {
		/* Not supported on this platform */
		return NULL;
	}
	return usbi_backend->device_with_desc(ctx, dev_node, desc, desc_length);
}
