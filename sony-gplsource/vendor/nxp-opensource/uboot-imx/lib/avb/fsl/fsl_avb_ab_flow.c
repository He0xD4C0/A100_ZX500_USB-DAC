		...
		struct spl_image_info *spl_image, struct mmc *mmc)
{
	...
	for (n = 0; n < 2; n++)
		printf("[avb]slot%lu priority:%d successful_boot:%d tries_remaining:%d\n",
			        (unsigned long)n, ab_data.slots[n].priority, ab_data.slots[n].successful_boot, ab_data.slots[n].tries_remaining);
	for (n = 0; n < 2; n++) {
		if (!fsl_slot_is_bootable(&ab_data.slots[target_slot])) {
			target_slot = (target_slot == 1 ? 0 : 1);
			continue;
	...
	if (slot_index_to_boot == 2) {
		/* No bootable slots! */
		printf("No bootable slots found.\n");
		memset(ab_data.magic, 0, AVB_AB_MAGIC_LEN);
		fsl_save_metadata_if_changed_dual_uboot(dev_desc, &ab_data, &ab_data_orig);
		ret = -1;
		goto end;
	} else if (!ab_data.slots[slot_index_to_boot].successful_boot &&
		   (ab_data.slots[slot_index_to_boot].tries_remaining > 0)) {
...
AvbABFlowResult avb_flow_dual_uboot(AvbABOps* ab_ops,
	...
	char *bootmode = env_get("bootmode");
	...
	*/
	...
	if ((ab_data.slots[target_slot].successful_boot == 0)
	    && (bootmode != NULL)
	    && ( !strncmp(bootmode, "charger", sizeof("charger"))
		|| !strncmp(bootmode, "diag", sizeof("diag"))
		|| !strncmp(bootmode, "init", sizeof("init")))) {
		ab_data.slots[target_slot].tries_remaining += 1;
		printf("[avb]Increase slot:%lu tries_remaining:%d\n",
		        (unsigned long)target_slot, ab_data.slots[target_slot].tries_remaining);
	}
out:
	...
	*/
	...
	if (!ab_data.slots[slot_index_to_boot].successful_boot &&
		...
		if ((bootmode == NULL)
			...
			&& strncmp(bootmode, "charger", sizeof("charger")))) {
			ab_data.slots[slot_index_to_boot].tries_remaining -= 1;
			printf("[avb] Decrease slot:%lu tries_remaining:%d\n",
			        (unsigned long)slot_index_to_boot, ab_data.slots[slot_index_to_boot].tries_remaining);
		}
	}
