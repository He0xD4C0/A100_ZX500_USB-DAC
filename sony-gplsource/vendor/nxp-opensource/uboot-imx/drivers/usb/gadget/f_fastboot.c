...
#ifdef CONFIG_ICX_UNLOCKED_LOGO
#include <dmp_logo.h>
#endif
...
enum {
	...
	PTN_TEE_INDEX,
#ifdef CONFIG_FLASH_MCUFIRMWARE_SUPPORT
#ifndef CONFIG_MCU_FIRMWARE_AVB_SUPPORT
	PTN_M4_OS_INDEX,
#endif
#endif
	PTN_ALL_INDEX,
...
#ifdef CONFIG_MCU_FIRMWARE_AVB_SUPPORT
	if (argc != 2) {
		printf("bootmcu invalid args\n");
		return 1;
	}
	printf("loading m4 from %s\n", argv[1]);
	part_name = argv[1];
#else
	part_name = FASTBOOT_MCU_FIRMWARE_PARTITION;
#endif
	ret = read_from_partition_multi(part_name,
...
#ifdef CONFIG_MCU_FIRMWARE_AVB_SUPPORT
U_BOOT_CMD(
	bootmcu, 2, 0, do_bootmcu,
	"boot mcu images from partition\n",
	"boot mcu images from given partition, only support images run from TCM"
);
#else
U_BOOT_CMD(
...
#ifdef CONFIG_FLASH_MCUFIRMWARE_SUPPORT
#ifdef CONFIG_MCU_FIRMWARE_AVB_SUPPORT
	if (!strncmp(cmdbuf, FASTBOOT_MCU_FIRMWARE_PARTITION "_a",
				sizeof(FASTBOOT_MCU_FIRMWARE_PARTITION)) ||
		!strncmp(cmdbuf, FASTBOOT_MCU_FIRMWARE_PARTITION "_b",
				sizeof(FASTBOOT_MCU_FIRMWARE_PARTITION))) {
#else
...
#if defined(CONFIG_CMD_BOOT_CM4) && \
	!defined(CONFIG_MCU_FIRMWARE_AVB_SUPPORT)
		sprintf(boot_dev_part, "bootmcu && boota mmc%d", dev_no);
#else
...
#if defined(CONFIG_CMD_BOOT_CM4) && \
	!defined(CONFIG_MCU_FIRMWARE_AVB_SUPPORT)
		env_set("bootcmd_android_recovery", "bootmcu && boota recovery");
#else
...
#if defined(CONFIG_CMD_BOOT_CM4) && \
	!defined(CONFIG_MCU_FIRMWARE_AVB_SUPPORT)
		sprintf(boot_dev_part,"bootmcu && boota mmc%d recovery",dev_no);
#else
...
static FbBootMode fastboot_get_bootmode(void)
{
	...
	(void) icx_key_pressing_probe();
	/* Ignore error. */
#endif /* (defined(CONFIG_TARGET_IMX8MM_DMP1)) */
...
#if (defined(CONFIG_ANDROID_RECOVERY) && \
     defined(CONFIG_SUPPORT_SERVICE_MODE) && defined(TARGET_BUILD_VARIANT_USERDEBUG))
		if (is_recovery_service_mode())
			boot_mode = BOOTMODE_RECOVERY_SERVICE_MODE;
#endif /* CONFIG_ANDROID_RECOVERY && CONFIG_SUPPORT_SERVICE_MODE */
		return boot_mode;
	}
...
#endif
#if (defined(CONFIG_ICX_FASTBOOT_MAGIC_KEY) && defined(TARGET_BUILD_VARIANT_USERDEBUG))
	else if (!strcmp(command, FASTBOOT_FORCE_BCB_CMD)) {
		icx_fastboot_force_unlock();
		boot_mode = BOOTMODE_FASTBOOT_BCB_CMD;
	}
#endif /* (defined(CONFIG_ICX_FASTBOOT_MAGIC_KEY)) */
...
#if (defined(CONFIG_ANDROID_RECOVERY) && \
     defined(CONFIG_SUPPORT_SERVICE_MODE) && defined(TARGET_BUILD_VARIANT_USERDEBUG))
	if ((boot_mode == BOOTMODE_NORMAL) && is_recovery_service_mode())
		boot_mode = BOOTMODE_RECOVERY_SERVICE_MODE;
#endif /* CONFIG_ANDROID_RECOVERY && CONFIG_SUPPORT_SERVICE_MODE */
	return boot_mode;
}
...
void fastboot_run_bootmode(void)
{
	...
	case BOOTMODE_RECOVERY_SERVICE_MODE:
		/* Make the boot into recovery mode */
		puts("Fastboot: Got Recovery setting to boot up service mode!\n");
		board_recovery_setup();
		break;
#endif
#endif
...
#else
	sha256_csum_wd((unsigned char *)fsl_public_key, AVB_SHA256_DIGEST_SIZE,
...
static struct dt_table_entry *get_dt_entry(struct dt_table_header *hdr, u32 entry_idx)
{
	struct dt_table_entry *fdt_entry;
	u32 entry_count, entries_offset, entry_size, entry_id;

	entry_count = be32_to_cpu(hdr->dt_entry_count);
	entries_offset = be32_to_cpu(hdr->dt_entries_offset);
	entry_size = be32_to_cpu(hdr->dt_entry_size);

	if (entry_idx > entry_count - 1) {
		printf("Error: entry_idx > entry_count (0x%x > 0x%x), Use default entry_idx.\n",
			entry_idx, entry_count);
		entry_idx = 0;
	}

	fdt_entry = (struct dt_table_entry *)((ulong)hdr + entries_offset + entry_idx * entry_size);
	entry_id = be32_to_cpu(fdt_entry->id);
	if (entry_idx != entry_id) {
		printf("Error: entry_idx != fdt_entry id (0x%x != 0x%x), Use default fdt_entry.\n",
			entry_idx, entry_id);
		fdt_entry = (struct dt_table_entry *)((ulong)hdr + entries_offset);
	}

	printf("%s: counts=%u idx=0x%x id=0x%08x\n", __func__,
		entry_count, entry_idx, entry_id);
...
#ifdef CONFIG_MCU_FIRMWARE_AVB_SUPPORT
	snprintf(command, 32, "bootmcu %s%s", FASTBOOT_MCU_FIRMWARE_PARTITION,
		avb_out_data->ab_suffix);
	ret = run_command(command, 0);
	if (ret)
		printf("run 'bootmcu' command %s failed!\n", command);
#endif

#ifdef CONFIG_ICX_UNLOCKED_LOGO
	if (fastboot_get_lock_stat() == FASTBOOT_UNLOCK) {
		if (display_dmp_logo_file(env_get("unlocked_logo"))) {
			snprintf(part_name, 32, "bootloader%s", avb_out_data->ab_suffix);
			display_dmp_logo_offset(part_name,
				CONFIG_ICX_UNLOCKED_LOGO_OFFSET,
				CONFIG_ICX_UNLOCKED_LOGO_COUNT);
		}
		mdelay(10000); //10s wait
	}
#endif
...
#ifndef CONFIG_DISABLE_UUU_COMMAND
